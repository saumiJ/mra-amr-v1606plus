...
Foam::hexTetRef8Data::hexTetRef8Data(const IOobject& io)
{
    {
        IOobject rio(io);
    ...
    {
        IOobject rio(io);
        rio.rename("refinementMRAHistory");
        bool haveFile = returnReduce
        (
            rio.typeHeaderOk<labelIOList>(true),
            orOp<bool>()
        ...
        {
            Info<< "Reading hexRef8 data : " << rio.name() << endl;
            refHistoryPtr_.reset(new refinementMRAHistory(rio));
        }
    }
}


Foam::hexTetRef8Data::hexTetRef8Data
(
    const IOobject& io,
    const hexTetRef8Data& data,
...
(
    ...
    const UPtrList<const hexTetRef8Data>& procDatas
)
{
    ...
    if (procDatas[0].refHistoryPtr_.valid())
    {
        ...
        UPtrList<const refinementMRAHistory> procRefs(procDatas.size());
        forAll(procDatas, i)
        {
...
void Foam::hexTetRef8Data::sync(const IOobject& io)
{
    const polyMesh& mesh = dynamic_cast<const polyMesh&>(io.db());
    ...
    if (hasHistory && !refHistoryPtr_.valid())
    {
        ...
        rio.rename("refinementMRAHistory");
        rio.readOpt() = IOobject::NO_READ;
        refHistoryPtr_.reset(new refinementMRAHistory(rio, mesh.nCells(), true));
    }
}


void Foam::hexTetRef8Data::distribute(const mapDistributePolyMesh& map)
{
...
bool Foam::hexTetRef8Data::write() const
{
    bool ok = true;
    if (cellLevelPtr_.valid())
    {
